...
namespace Codecool.MissingDog.Repository
{
    ...
    public class OwnerRepository
    {
        ...
        {
            return _data.Owners.FirstOrDefault(o => o?.Id == id);
        }
        ...
        {
            return GetOwnerById(ownerId)?.Dogs?.Count(dog => dog?.Breed?.Id == breedId) ?? 0;
        }
        ...
        public double CountAverageActivityLevelOfThisOwnersDogs(int ownerId)
        {
            return GetOwnerById(ownerId)?
                ...
                .Select(d => d.Breed.ActivityLevel).Average() ?? 0;
        }
        ...
        {
            return GetOwnerById(ownerId)?.Dogs?.Where(d => d?.DateOfBirth != null).OrderBy(d => d.DateOfBirth)
                .FirstOrDefault();
        }
        ...
        {
            return GetOwnerById(ownerId)?.Dogs?.Count(d => d != null) ?? 0;
        }
    }
}
